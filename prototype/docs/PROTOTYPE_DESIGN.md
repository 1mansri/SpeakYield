...
Six core screens, one optional stretch screen, plus a testing-only login gate. Numbers match the flow order.

> **Login (testing-only, not counted among the six core screens).** A plain ID/password form in front of the whole flow, backed by a fixed mock user dataset (`backend/app/data/users.json`) — lets different testers sign in as different personas and confirms real frontend↔backend network calls work. It deliberately does **not** follow the voice-first/icon-driven/multilingual design principles in §1 above (plain English labels, standard text inputs) because it is not part of the farmer-facing product — the real MVP has no login at all, per [PROTOTYPE.md §3](./PROTOTYPE.md#3-scope). See `backend/app/routers/auth.py`.
...
| `POST` | `/api/auth/login` | **Testing-only, not part of the farmer-facing product.** `{ id, password }` checked against the fixed mock user dataset → `{ token, user }`. Plaintext-compared, in-memory session store — not real security, exists only to gate/demo the app for testers. |
| `GET` | `/api/auth/me` | **Testing-only.** `Authorization: Bearer <token>` → the logged-in mock user's profile. |
| `GET` | `/api/catalog` | Returns the fixed sample dataset (buyers, dealers, seed listings) — debug/inspection use. |
| `POST` | `/api/voice/transcribe` | Multipart audio upload → proxied to Sarvam AI's Saaras STT API → `{ transcript, language }`. |
| `POST` | `/api/voice/intent` | `{ transcript, language }` → Sarvam AI chat completions → structured `{ action: "sell"\|"buy", commodity, quantity, unit, price, location, confidence }`. |
| `POST` | `/api/voice/speak` | `{ text, language }` → Sarvam AI Bulbul TTS → audio stream, used for the Confirm Draft read-back. |
...
Eight product routes plus two testing-only auth routes — the product surface stays deliberately minimal and matches the six-screen flow with no unused endpoints; `/api/auth/*` exists solely to gate the prototype for testers, not as part of the designed product. Full OpenAPI contract is auto-generated by FastAPI at `/docs` once the backend is running.
